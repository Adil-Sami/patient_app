...
class WalkinReschConfirmationPage extends StatefulWidget {
  WalkinReschConfirmationPage({Key? key}) : super(key: key);

  @override
  _WalkinReschConfirmationPageState createState() =>
      _WalkinReschConfirmationPageState();
}

class _WalkinReschConfirmationPageState
    extends State<WalkinReschConfirmationPage> {
  String _adminFCMid = "";
  String doctorFcm = "";
  ...
  _handleUpdate() async {
    ...
    //     _patientDetailsArgs.doctId);
    //  if (res == "success") {
    final getDralkinslot = await DrProfileService.getDataByDrId(
        _patientDetailsArgs.doctId);
    ...
    // print("8888888888888888myresx ${cehckWalkin.length} $walkinslots");
    if (cehckWalkin.length < walkinslots) {
      ...
      ToastMsg.showToastMsg(
          "Walk-in slots for the selected doctor is already finished for today".tr);
    }
